#!/usr/bin/env python3
"""Correct v12 nested assembly-script literals and execute the audited builder."""
import requests
...
replacement='''        urls="\\n".join([f"  '{p['url']}'," for p in parts]).rstrip(',')
        ...
        sh_lines.extend([f"curl -fL '{p['url']}' >> \\"$out\\"" for p in parts])
        sh_lines.extend(["actual=$(sha256sum \\"$out\\" | awk '{print $1}')",f'expected="{digest}"',"printf 'SHA-256: %s\\\\n' \\"$actual\\"",'[ "$actual" = "$expected" ] || { echo \'SHA-256 mismatch\' >&2; exit 1; }','echo "Archive assembled and verified: $out"'])
        sh="\\n".join(sh_lines)+"\\n"
        readme="\\n".join([
