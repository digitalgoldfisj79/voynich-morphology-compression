...
def even_take(items,n):
    if len(items)<n:
        raise RuntimeError(f'need {n} items, have {len(items)}')
    idx=np.linspace(0,len(items)-1,n).round().astype(int)
    return [items[int(i)] for i in idx]


def fetch_page_crops(manifest, inds, shelfmark, family):
    cs=ec.canvases(ec.get_json(manifest)); all_items=[]
    for pi in inds:
        im=ec.fetch_image(ec.image_url(cs[pi])); bw=ec.ink_mask(im); boxes=ec.word_boxes(bw)
...
def main():
    ...
    vcs=ec.canvases(ec.get_json(spec['vms_manifest']))
    ...
    out=Path('/tmp/dino_input_bank.npz')
    np.savez_compressed(out,images=A,meta=meta_json)
    sha=hashlib.sha256(out.read_bytes()).hexdigest()
    audit={
      'protocol':'2026-09-01.greek-ductus.dino-replication1',
      'input_sha256':sha,
      'shape':list(A.shape),
      'n':len(A),
      'controls':sum(m['family']!='VOYNICH' for m in meta),
      'voynich':sum(m['family']=='VOYNICH' for m in meta),
      'families':{f:sum(m['family']==f for m in meta) for f in ['GREEK','LATIN','VOYNICH']},
      'source_manifest':'replication1_manifest.json'
    }
